...
export default function AdminDashboardLayout() {
  ...
  useEffect(() => {
    const currentItem = menuItems[0].items.find((item) => item.path === location.pathname)
    if (currentItem) {
      ...
      <aside className={`hidden lg:block ${isCollapsed ? "w-20" : "w-80"} transition-all duration-500 ease-in-out`}>
        ...
        <div className="h-auto flex items-center px-4">
          <div className="flex flex-col w-full justify-center items-center mt-16">
            ...
            >
              <img src={UserAvatar || "/placeholder.svg"} alt="User" className="w-16 h-16 rounded-full" />
            </div>

            <div className="w-full flex flex-col gap-1 pl-3">
              <h3 className="text-2xl text-center font-normal text-[#343E4B]">Company Profile</h3>
              <span className="text-center text-md text-[#8C8C8C]">username</span>
            </div>
          </div>
        </div>
        ...
        <nav className="p-4 mt-6">
                  ...
                  <li key={itemIdx}>
                    <Link
                      ...
                      className={`flex items-center gap-3 px-3 py-2 text-[#67748E] rounded-lg group relative ${
                        selectedItem === item.name ? "bg-[#3776E2] text-white font-semibold" : ""
                      }`}
                    >
          ...
          <div className="h-44 w-full flex flex-col bg-gradient-to-br from-blue-500 to-purple-600 bg-no-repeat bg-center p-5 gap-2 rounded-2xl">
            ...
            <div className="w-full flex flex-col gap-1">
              <h3 className="font-open-sans text-base font-semibold text-white">Need Help</h3>
              <h5 className="font-open-sans text-sm font-normal text-white">Please check our docs</h5>
        ...
        <div className="h-auto flex items-center px-4">
          <div className="flex flex-col w-full justify-center items-center mt-8">
            ...
            <div className="w-full flex flex-col gap-1 pl-3 mt-4">
              <h3 className="text-xl text-center font-normal text-[#343E4B]">Company Profile</h3>
              <span className="text-center text-sm text-[#8C8C8C]">username</span>
            </div>
          </div>
        </div>
                ...
                {section.items.map((item, itemIdx) => (
                  <li key={itemIdx}>
                    ...
                    >
                      ...
                      <span className="text-md font-semibold whitespace-nowrap">{item.name}</span>
                      {item.badge && (
                        <span className="ml-auto bg-red-500 text-white text-xs font-semibold rounded-full px-2 py-0.5">
                          {item.badge}
                        </span>
